...
| `join_token` | `REDACTED_SPIRE_TOKEN_1` | `spire/agent/agent.conf` |
...
- **SPIRE join token** (`REDACTED_SPIRE_TOKEN_1`) is stored in plaintext in `spire/agent/agent.conf`. This is the initial bootstrap token and should be rotated after first attestation.
