...
`insecure_bootstrap: true` was set for initial setup and the join token (`6087b957-918f-4eed-a0da-813dcf887f10`) is plaintext in the config. After initial agent attestation the trust bundle should be pinned, `insecure_bootstrap` disabled, and the token rotated or replaced with a more durable attestation method (e.g., x509pop or TPM).
